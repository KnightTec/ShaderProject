...
float henyeyGreensteinPhaseFunctionSchlick(float3 pointPosition, float3 lightDirection, float3 camPos, float k) 
{
	float3 viewDir = normalize(pointPosition - camPos);
	float theta = dot(lightDirection, viewDir);
	float phase = 1 + k * theta;
	return (1 - k * k) / (4 * phase * phase);
}
